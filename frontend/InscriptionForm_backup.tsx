...
export default function InscriptionForm({ onSuccess }: { onSuccess?: (data: any) => void }) {
  ...
  const [formData, setFormData] = useState({
    nom: '', prenom: '', adresse: '', ville_id: '', telGsm: '', telFixe: '', email: '',
    activitePrincipale: '', activiteSecondaire: '',
    raisonSociale: '', abreviation: '', siteWeb: '', facebook: '',
    dateCreation: '', ice: '', rc: '', capital: '', ca_id: '',
    effectif_id: '', pourcentageEtrangers: '', nationalite: '',
            ...
            <div className="card" style={{ marginBottom: '24px' }}>
              <div className="card-body">
                ...
                <div className="grid-3">
                  <div className="form-group"><label>Ville</label><select name="ville_id" value={formData.ville_id} onChange={handleInputChange} style={selectStyle}><option value="">Sélectionner...</option>{villes.map((v:any) => <option key={v.id} value={v.id}>{v.titre}</option>)}</select></div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        {(services as any[]).map((service: any) => (
                          <label key={service.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', cursor: 'pointer' }}>
                            ...
                            />
                            <span style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>{service.titre || service.nom_salle}</span>
                          </label>
                        ))}
                      </div>
                    </div>
